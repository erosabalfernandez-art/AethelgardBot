#!/usr/bin/env python3
# investigacion_amarilla.py
# Generado automáticamente por generar_investigacion.py para zona amarilla
...
ZONAS_INFO = {
    'Desierto Alianza': {'zona_id': 3, 'nivel': 20, 'color': 'amarilla'},
    'Ruinas Alianza': {'zona_id': 4, 'nivel': 25, 'color': 'amarilla'},
    'Oasis Alianza': {'zona_id': 5, 'nivel': 30, 'color': 'amarilla'},
    'Desierto Imperio': {'zona_id': 14, 'nivel': 20, 'color': 'amarilla'},
    'Ruinas Imperio': {'zona_id': 15, 'nivel': 25, 'color': 'amarilla'},
    'Oasis Imperio': {'zona_id': 16, 'nivel': 30, 'color': 'amarilla'},
    'Desierto Sindicato': {'zona_id': 25, 'nivel': 20, 'color': 'amarilla'},
    'Ruinas Sindicato': {'zona_id': 26, 'nivel': 25, 'color': 'amarilla'},
    'Oasis Sindicato': {'zona_id': 27, 'nivel': 30, 'color': 'amarilla'},
}
...
async def cmd_investigar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("👣 Seguir huellas", callback_data=f"investigar_huellas_amarilla_{zona_nombre}")],
...
async def seguir_huellas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    zona_nombre = query.data.replace("investigar_huellas_amarilla_", "", 1)
    user_id = update.effective_user.id
    if db_helper.esta_marcado(user_id):
...
async def meditar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    zona_nombre = query.data.replace("investigar_meditar_amarilla_", "", 1)
    user_id = update.effective_user.id
    if db_helper.esta_marcado(user_id):
...
def registrar_handlers(app):
    ...
    app.add_handler(CallbackQueryHandler(seguir_huellas, pattern="^investigar_huellas_amarilla_"))
