#!/usr/bin/env python3
# investigacion_azul.py
# Generado automáticamente por generar_investigacion.py para zona azul
...
ZONAS_INFO = {
    'Bosque Alianza': {'zona_id': 2, 'nivel': 1, 'color': 'azul'},
    'Bosque Imperio': {'zona_id': 13, 'nivel': 1, 'color': 'azul'},
    'Bosque Sindicato': {'zona_id': 24, 'nivel': 1, 'color': 'azul'},
}
...
async def cmd_investigar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("👣 Seguir huellas", callback_data=f"investigar_huellas_azul_{zona_nombre}")],
        [InlineKeyboardButton("🧘‍♂️ Meditar", callback_data=f"investigar_meditar_azul_{zona_nombre}")],
...
async def seguir_huellas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    zona_nombre = query.data.replace("investigar_huellas_azul_", "", 1)
    user_id = update.effective_user.id
    if db_helper.esta_marcado(user_id):
    ...
    try:
        ...
        _mis.completar_mision(user_id, "primera_investigacion", context)
    except Exception:
        pass
...
async def meditar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    zona_nombre = query.data.replace("investigar_meditar_azul_", "", 1)
    user_id = update.effective_user.id
    if db_helper.esta_marcado(user_id):
...
def registrar_handlers(app):
    ...
    app.add_handler(CallbackQueryHandler(seguir_huellas, pattern="^investigar_huellas_azul_"))
    app.add_handler(CallbackQueryHandler(meditar, pattern="^investigar_meditar_azul_"))
